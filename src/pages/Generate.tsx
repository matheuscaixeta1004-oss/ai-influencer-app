...
const scenarios = [
  { id: 'beach', emoji: '🏖️', label: 'Beach', prompt: 'Relaxing at a tropical beach, golden hour, bikini, ocean waves' },
  { id: 'gym', emoji: '💪', label: 'Gym', prompt: 'Post-workout selfie at the gym, sports bra, sweaty, confident' },
  { id: 'cafe', emoji: '☕', label: 'Café', prompt: 'Sitting at a trendy coffee shop, casual outfit, latte, warm light' },
  { id: 'nightout', emoji: '🍸', label: 'Night Out', prompt: 'Rooftop bar at night, cocktail dress, city lights, glamorous' },
  { id: 'bedroom', emoji: '🛏️', label: 'Bedroom', prompt: 'Morning in bed, oversized shirt, natural light, cozy intimate' },
  { id: 'urban', emoji: '🏙️', label: 'Street', prompt: 'Walking down city street, trendy outfit, shopping bags, candid' },
  { id: 'pool', emoji: '🏊', label: 'Pool', prompt: 'Luxury pool, swimsuit, palm trees, tropical villa, summer vibes' },
  { id: 'studio', emoji: '📸', label: 'Studio', prompt: 'Professional studio portrait, dramatic lighting, high fashion' },
  { id: 'mirror', emoji: '🪞', label: 'Mirror', prompt: 'Mirror selfie, stylish outfit, walk-in closet, full body' },
  { id: 'dinner', emoji: '🍷', label: 'Dinner', prompt: 'Candlelit restaurant dinner, elegant dress, wine glass, romantic' },
  { id: 'bath', emoji: '🛁', label: 'Bath', prompt: 'Bubble bath, candles, champagne glass, relaxing luxury' },
  { id: 'cooking', emoji: '🍳', label: 'Kitchen', prompt: 'Cooking in kitchen, casual wear, morning vibes, playful' },
];
          ...
          <AnimatePresence mode="wait">
            {isGenerating ? (
              ...
              >
                ...
                <p className="text-[15px] font-medium text-black">Generating...</p>
                <p className="text-[13px] text-gray-400 mt-1">~15-30 seconds</p>
              ...
              >
                ...
                <p className="text-[16px] font-medium text-gray-800">Create something amazing</p>
                <p className="text-[13px] text-gray-400 mt-1.5 max-w-xs">Describe a scene or pick a scenario below, then hit Generate</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
          ...
          >
            ...
            <div className="flex items-start gap-3 px-4 pt-4 pb-3">
              <textarea
                ...
                placeholder="Describe the scene, or pick a scenario..."
                rows={2}
                  ...
                  <div className="relative">
                    ...
                    <AnimatePresence>
                      {showModelPicker && (
                        ...
                        >
                          <p className="text-[10px] font-semibold tracking-[0.12em] uppercase text-gray-400 px-2 py-1.5">Character</p>
                          {activeModels.map((m) => (
                            <button
                              key={m.id}
                  ...
                  {showSettings && (
                    ...
                    >
                      {(['standard', 'hd'] as const).map((q) => (
                        ...
                        >
                          {q === 'standard' ? '1K · Standard' : 'HD · High Def'}
                          <span className="block text-[10px] text-gray-400 mt-0.5">{q === 'standard' ? `${CREDITS.GENERATE_STANDARD} credits` : `${CREDITS.GENERATE_HD} credits`}</span>
                        </button>
                      ))}
                    </motion.div>
                  )}
